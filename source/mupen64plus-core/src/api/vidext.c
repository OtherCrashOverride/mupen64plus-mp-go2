...
EXPORT m64p_error CALL VidExt_Init(void)
{
    ...
    encoder = gs_encoder_session_create(0, 0, 60, 44100);
    if (!encoder)
    {        
        DebugMessage(M64MSG_ERROR, "gs_encoder_session_create failed.");
        return M64ERR_SYSTEM_FAIL;
    }
...
EXPORT m64p_error CALL VidExt_GL_SwapBuffers(void)
{
    ...
    if (gs_network_client_count_get(encoder) > 0)
    {
        if (firstFrameFlag)
        {
            ...
            memcpy(value, &aspect_ratio, sizeof(aspect_ratio));

            if (!encoder) abort();
            gs_encoder_session_value_set(encoder, GS_ENCODER_KEY_ASPECT_RATIO, value);
