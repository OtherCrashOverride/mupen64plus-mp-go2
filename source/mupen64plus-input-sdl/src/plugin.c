...
typedef gs_encoder_session_t* (*get_encoder_proc)();
get_encoder_proc get_encoder;
                                   ...
                                   void (*DebugCallback)(void *, int, const char *))
{
    ...
    get_encoder = (get_encoder_proc)osal_dynlib_getproc(CoreLibHandle, "get_encoder");
    if (!get_encoder)
    {
        DebugMessage(M64MSG_ERROR, "get_encoder symbol not found.");
...
EXPORT void CALL GetKeys( int Control, BUTTONS *Keys )
{
    ...
    else if (Control == 1)
    {
        ...
        if (encoder && gs_network_client_count_get(encoder) > 0)
        {
            network_gamepad_t networkGamepad;
            gs_encoder_session_gamepad_read(encoder, 0, &networkGamepad);

            Keys->START_BUTTON = networkGamepad.f4 ? 1 : 0;

            Keys->A_BUTTON = networkGamepad.b ? 1 : 0;
            Keys->B_BUTTON = networkGamepad.y ? 1 : 0;

            Keys->L_DPAD = networkGamepad.left ? 1 : 0;
            Keys->R_DPAD = networkGamepad.right ? 1 : 0;
            Keys->U_DPAD = networkGamepad.up ? 1 : 0;
            Keys->D_DPAD = networkGamepad.down ? 1 : 0;

            Keys->L_CBUTTON = networkGamepad.x ? 1 : 0;
            Keys->R_CBUTTON = networkGamepad.f6 ? 1 : 0;
            Keys->U_CBUTTON = networkGamepad.f5 ? 1 : 0;
            Keys->D_CBUTTON = networkGamepad.a ? 1 : 0;

            Keys->R_TRIG = networkGamepad.r1 ? 1 : 0;
            Keys->Z_TRIG = networkGamepad.l1 ? 1 : 0;
            Keys->L_TRIG = networkGamepad.f3 ? 1 : 0;

            Keys->X_AXIS = networkGamepad.thumb_x * 80;
            Keys->Y_AXIS = networkGamepad.thumb_y * -80;
        }
    }
...
EXPORT void CALL InitiateControllers(CONTROL_INFO ControlInfo)
{
    ...
    controller[1].control->Plugin = PLUGIN_MEMPAK;
